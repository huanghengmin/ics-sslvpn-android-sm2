...
public class ConfigParser {
    ...
    public VpnProfile convertProfile() throws ConfigParseError, IOException {
        ...
        Vector<String> extra_certs = getOption("extra-certs", 1, 1);
        if (extra_certs != null) {
            np.mClientEncCertFilename = extra_certs.get(1);
            np.mAuthenticationType = VpnProfile.TYPE_CERTIFICATES;
            noauthtypeset = false;
        }
        ...
        Vector<String> enc_key = getOption("enc-key", 1, 1);
        if (enc_key != null)
            np.mClientEncKeyFilename = enc_key.get(1);
