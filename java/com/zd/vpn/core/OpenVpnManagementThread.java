...
public class OpenVpnManagementThread implements Runnable, OpenVPNManagement {
        ...
        if (shouldBeRunning()) {
            if (waittime > 1)
                VpnStatus.updateStateString("CONNECTRETRY", String.valueOf(waittime),
                        com.zd.vpn.R.string.state_waitconnectretry, ConnectionStatus.LEVEL_CONNECTING_NO_SERVER_REPLY_YET);
            mResumeHandler.postDelayed(mResumeHoldRunnable, waittime * 1000);
            if (waittime > 5) {
                VpnStatus.logInfo(com.zd.vpn.R.string.state_waitconnectretry, String.valueOf(waittime));
    ...
    private void processProxyCMD(String argument) {
        ...
        if (proxyaddr instanceof InetSocketAddress) {
            ...
            VpnStatus.logInfo(com.zd.vpn.R.string.using_proxy, isa.getHostName(), isa.getPort());
    ...
    private void proccessPWFailed(String needed, String args) {
        VpnStatus.updateStateString("AUTH_FAILED", needed + args, com.zd.vpn.R.string.state_auth_failed, ConnectionStatus.LEVEL_AUTH_FAILED);
    }
