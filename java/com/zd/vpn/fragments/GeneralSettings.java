...
import com.zd.vpn.api.ExternalAppDatabase;
...
public class GeneralSettings extends PreferenceFragment implements OnPreferenceClickListener, OnClickListener, Preference.OnPreferenceChangeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(com.zd.vpn.R.xml.general_settings);
	...
	public void onResume() {
        ...
        if (vpn== null)
            mAlwaysOnVPN.setSummary(com.zd.vpn.R.string.novpn_selected);
        else
    ...
    private void setClearApiSummary() {
		...
		if(mExtapp.getExtAppList().isEmpty()) {
			...
			clearapi.setSummary(com.zd.vpn.R.string.no_external_app_allowed);
		} else { 
			clearapi.setEnabled(true);
			clearapi.setSummary(getString(com.zd.vpn.R.string.allowed_apps,getExtAppList(", ")));
		}
	}
	...
	public boolean onPreferenceClick(Preference preference) { 
		if(preference.getKey().equals("clearapi")){
			...
			builder.setPositiveButton(com.zd.vpn.R.string.clear, this);
			builder.setNegativeButton(android.R.string.cancel, null);
			builder.setMessage(getString(com.zd.vpn.R.string.clearappsdialog,getExtAppList("\n")));
			builder.show();
		}
