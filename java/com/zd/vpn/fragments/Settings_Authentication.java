...
import com.zd.vpn.views.RemoteCNPreference;
...
public class Settings_Authentication extends OpenVpnPreferencesFragment implements OnPreferenceChangeListener, OnPreferenceClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		addPreferencesFromResource(com.zd.vpn.R.xml.vpn_authentification);
		...
		if(preference==mRemoteCN) {
			...
			if ("".equals(dn)) {
                if (mProfile.mConnections.length > 0) {
                    ...
                    preference.setSummary(com.zd.vpn.R.string.no_remote_defined);
                }
            } else {
    ...
    void startFileDialog() {
        ...
        if (startFC == null) {
            ...
            startFC.putExtra(FileSelect.WINDOW_TITLE, com.zd.vpn.R.string.tls_auth_file);
	...
	private void setTlsAuthSummary(String result) {
		if(result==null)
            result = getString(com.zd.vpn.R.string.no_certificate);
		if(result.startsWith(VpnProfile.INLINE_TAG))
			mTLSAuthFile.setSummary(com.zd.vpn.R.string.inline_file_data);
        else if (result.startsWith(VpnProfile.DISPLAYNAME_TAG))
            mExpectTLSCert.setSummary(getString(com.zd.vpn.R.string.imported_from_file, VpnProfile.getDisplayName(result)));
		else
